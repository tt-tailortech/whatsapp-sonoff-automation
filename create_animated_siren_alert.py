...
def create_animated_emergency_alert_gif(
    ...
    incident_type: str = "EMERGENCIA GENERAL",  # Default changed, moved after required params
    ...
    output_path = f"/Users/bmac/Library/CloudStorage/OneDrive-TheUniversityofMemphis/Other/TT/Alarm_system/animated_emergency_alert_{timestamp_file}.gif"
...
def draw_spinning_siren(draw, center_x, center_y, radius, rotation_angle, colors, frame_num):
    ...
    beam_end_x = center_x + beam_length * math.cos(angle_rad)
    beam_end_y = center_y + beam_length * math.sin(angle_rad)
...
def test_animated_siren_alert():
    """Test the animated siren emergency alert"""
    
    print("🚨 Testing ANIMATED SIREN Emergency Alert")
    print("=" * 60)
    print("🌟 Creating spinning siren animation GIF")
    ...
    try:
        ...
        if os.path.exists(gif_path):
            ...
            print(f"   📱 All other elements remain static and professional")
            print(f"   🌙 Beautiful night sky atmosphere")
            
            return gif_path
        else:
            print("❌ Failed to create animated siren alert")
            return None
...
if __name__ == "__main__":
    result = test_animated_siren_alert()
    if result:
        print(f"\n🏆 ANIMATED SIREN alert created successfully!")
        print(f"🚨 Eye-catching spinning siren with professional layout!")
    else:
        print(f"\n❌ Test failed!")